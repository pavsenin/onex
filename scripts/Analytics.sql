...
INNER JOIN Bets b
ON b.MatchID = m.ID
WHERE StartedAt < ReceivedAt
ORDER BY StartedAt, ReceivedAt
...
SELECT Distinct g.MatchID FROM Grouped g
...
SELECT l.Name, t1.Name, t2.Name, m.StartedAt, gt.MatchID, gt.BetTypeID, gt.BetParam, 
gv.Min, gv.Max, (gv.Max - gv.Min) * 100.0 / gv.Max AS Diff, 100.0 / gv.Min AS PMin, 100.0 / gv.Max AS Pmax,
...
INNER JOIN Teams t1 ON m.Team1ID = t1.ID
INNER JOIN Teams t2 ON m.Team2ID = t2.ID
WHERE m.ID IN (23034897, 23035187, 23829589)
ORDER BY Diff DESC